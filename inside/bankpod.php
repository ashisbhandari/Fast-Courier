...
function generatePDF($conn, $cn) {
    ...
    // Prepare the SQL query to fetch data for the provided CN No.
    $sql = "SELECT * FROM domesticinfo WHERE `CN No` = ?";
    ...
    $yStart = ($pdf->GetPageHeight() - 90) / 2; // Center alignment for Y-axis; adjust based on table height
...
if (isset($_GET['cn'])) {
    ...
    // Set the headers to output the PDF inline
    header('Content-Type: application/pdf');
    header('Content-Disposition: inline; filename="consignment.pdf"');
